...
apa_gt <- function(data, label, title, note = NULL) {
  table <- data |>
    gt::gt() |>
    gt::tab_header(
      title = gt::md(paste0("**", label, "**")),
      subtitle = gt::md(paste0("*", title, "*"))
    ) |>
...
reader_table <- function(data, label, title, note = NULL, digits = 2, p_cols = NULL) {
  apa_gt(
